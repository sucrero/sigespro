<?php
    ...
    $titulo = $_GET['tit'];//titulo del proyecto
    $objetivo = $_GET['obj'];//objetivo general del proyecto
    $anteproyecto = $_GET['ant'];//codigo del anteproyecto
    ...
    $consejo = $_GET['con'];//nombre del consejo comunal
    $problema = $_GET['pro'];//codigo problema
    $sector = $_GET['sec'];//codigo del sector
    $responsable = $_GET['res'];//codigo de responsable
    ...
    $sql = "SELECT * from anteproyecto WHERE idantep='".$anteproyecto."'";
    if($objPer->buscar($sql, $acceso)){
...
//    $fila[4] = $comunidad;
    ...
    $sql="select (cedpersona || '-' || nompersona || ' ' || apepersona) as responsable from personal_sector_comunidad where idsectorcomunidad='".$sector."'";
    if($objPer->buscar($sql, $acceso)){
    ...
    $sql="select cedestudiante, (nomestudiante || ' ' || apeestudiante) as estudiante from estudiante where idgrupo='".$ante['idgrupo']."'";
    if($objPer->buscar($sql, $acceso)){
        $i=0;
        do{
    ...
    $sql = "SELECT * from problema where idproblema='".$problema."' and seleccionado='1'";
    if($objPer->buscar($sql, $acceso)){
    ...
    $sql = "SELECT * from pnf WHERE idpnf='".$ante['idpnf']."'";
    if($objPer->buscar($sql, $acceso)){
    ...
    $relleno = '00000';
    $codFormado = $fila[16]['abrevpnf']."PROT".$fila[1].substr($relleno, count($ante['maximo'])).$ante['maximo'];
    ...
    class PDF extends PDF_MC_Table{
        function Header() {
            ...
            //Titulo
            $this->Cell(0, 10, html_entity_decode('PLANILLA &Uacute;NICA DE PROYECTO'), 0, 0, 'C');
        }
...
//            $this->Cell(180);
        ...
        function tablaGrupo($cont){
            ...
            for($i=0;$i<count($cont);$i++){
                ...
                $this->Cell($w[1], 7, strtoupper($nac).' - '.number_format($ced,'0','','.'), 1, 0, 'C');
        ...
        function tablaProblema($sel){
            ...
            $this->SetFont('Arial','IB',7);
            $this->Cell(0, 5,'DATOS DEL PROBLEMA SELECCIONADO', 0, 1, 'C');
            $this->SetFont('Arial','B',7);
        ...
        function observaciones($datos){
            ...
            $this->SetFont('Arial','',7);
            $this->MultiCell(0, 7,utf8_decode($datos), 0, 'J', FALSE);
        }
    }
