<?php
    ...
    $titulo = $_GET['tit'];//titulo del anteproyecto
//    $objetivo = $_GET['obj'];//objetivo del anteproyecto
    $diagnostico = $_GET['dia'];//codigo del diagnostico
    ...
    $sql = "SELECT * from diagnostico WHERE iddiagnostico='".$diagnostico."'";
    if($objPer->buscar($sql, $acceso)){
        $diag = $acceso->devolver_recordset();
    }else{
        $w=false;
    }
        
    $sql="SELECT * from sector_comunidad WHERE idsectorcomunidad='".$diag['idsectorcomunidad']."'";
    if($objPer->buscar($sql, $acceso)){
    ...
    $sql="select (cedpersona || '-' || nompersona || ' ' || apepersona) as responsable from personal_sector_comunidad where idsectorcomunidad='".$diag['idsectorcomunidad']."'
        AND statuspersona='2'";
    if($objPer->buscar($sql, $acceso)){
    ...
    $fila[9] = $diag['nomconsejocomunal'];
    
    $sql="select cedestudiante, (nomestudiante || ' ' || apeestudiante) as estudiante from estudiante where idgrupo='".$diag['idgrupo']."'";
    ...
    $sql = "SELECT * from problema where iddiagnostico='".$diagnostico."' and seleccionado='1'";
    if($objPer->buscar($sql, $acceso)){
    ...
    $sql = "SELECT * from pnf WHERE idpnf='".$diag['idpnf']."'";
    if($objPer->buscar($sql, $acceso)){
...
//    }
    ...
    $sql = "SELECT max(idantep) as maximo from anteproyecto";
    if($objPer->buscar($sql, $acceso)){
    ...
    $relleno = '00000';
    $codFormado = $fila[16]['abrevpnf']."ANTT".$fila[1].substr($relleno, count($ante['maximo'])).$ante['maximo'];
...
//    class PDF extends FPDF{
    class PDF extends PDF_MC_Table{
        function Header() {
            ...
            //Titulo
            $this->Cell(0, 10, html_entity_decode('PLANILLA &Uacute;NICA DE ANTEPROYECTO'), 0, 0, 'C');
        }
...
//            $this->Cell(180);
   ...
   //$pdf->SetMargins(15, 15, 15);
    ...
    $nombre = "Planilla Anteproyecto_".$codFormado;
    $pdf->Output($nombre,"I");
?>