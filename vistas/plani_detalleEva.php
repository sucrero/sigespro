<?php
    ...
    $sql = "SELECT * FROM evaluacion_proyecto WHERE idevaluacion='".$cod."'";
...
//            }
    ...
    class PDF extends PDF_MC_Table{
       ...
       function Footer() {
        ...
        function titulo($titulo,$nota){
//            $this->SetFont('Arial', '', 8);
            $this->Ln(15);
            $this->MultiCell(0, 5, strtoupper(html_entity_decode(utf8_decode($titulo['nomproyecto']))),0,'C');
            $this->Ln(5);
...
//            $this->Cell(180);
            ...
            $this->SetFont('Arial','',7);
            $this->Cell(15, 5,$romanos[$datos[7]['trayectoproy']-1], 0, 0, 'L');
            $this->SetFont('Arial','B',7);
            $this->Cell(45, 5,'TRIMESTRE: ', 0, 0, 'R');
            $this->SetFont('Arial','',7);
            $this->Cell(17, 5,$romanos[$datos[7]['trimestreproy']-1], 0, 1, 'L');
            $this->Ln(5);
...
//            $detalle = split('-', $datos[6]['responsable']);
            ...
            $this->SetFont('Arial','',7);
            $this->Cell(0, 5, cambiarFormatoFecha($datos[7]['fechaproy'],1), 1, 1, 'L');
            $this->SetFont('Arial','B',7);
        ...
        function observaciones($datos){
            ...
            $this->SetFont('Arial','',7);
            $this->MultiCell(0, 7,strtoupper(html_entity_decode(utf8_decode($datos['obserevaluacion']))), 0, 'J', FALSE);
        }
    }
    
    if(strtoupper($fila[0]['notadescriptiva']) == 'A'){
        $nota = 'APROBADO';
    }else{
        $nota = 'REPROBADO';
    }
   ...
   //$pdf->SetMargins(15, 15, 15);
    ...
    $pdf->AddPage();
    $pdf->titulo($fila[7],$nota);
    $pdf->contenido($fila);
    $pdf->tablaGrupo($fila[1]);
    $pdf->tablaComision($fila[3]);
    $pdf->observaciones($fila[0]);
